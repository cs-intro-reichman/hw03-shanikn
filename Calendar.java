...
public class Calendar {	
	...
	public static void main(String args[]) {
		...
		int yearNow=Integer.parseInt(args[0]);
        // int sundays=2; 
	    //// Write the necessary initialization code, and replace the condition
	    //// of the while loop with the necessary condition 
	 	dayOfWeek=which_day(1, 1, yearNow);
        month=1;
        while(month<13)//runs on 12 months(a year)
			{   

				nDaysInMonth=nDaysInMonth(month, yearNow);//number of days in this month according to year and month
				// while(dayOfMonth<=nDaysInMonth)
				for(int n=0;n<nDaysInMonth;n++)//runs on a given number of days(one month) 
				{
					if(dayOfWeek==1)
					{
						System.out.println(dayOfMonth+"/"+month+"/"+yearNow+" Sunday");
						// if(dayOfMonth==1)
						// {
						// 	sundays++;
						// }
					...
					{
						System.out.println(dayOfMonth+"/"+month+"/"+yearNow);
					}
        ...
        private static int which_day(int yearNow, int monthNow, int dayOfMonthNow)
        {
            while(year<=yearNow) 
		    {
			    month=1;
